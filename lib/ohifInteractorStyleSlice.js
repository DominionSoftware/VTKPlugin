...
function ohifInteractorStyleSlice(publicAPI, model) {
    ...
    publicAPI.handleLeftButtonPress = (callData) => {
        ...
        if (callData.shiftKey){
            publicAPI.startPan();
        } else if (!callData.shiftKey && !callData.controlKey) {
    ...
    publicAPI.moveSliceByWheel = (increment)=>{
        ...
        if (slice) {
            ...
            let ds = publicAPI.getDisplaySet();

            let studyDate = ds.images[0]._study.studyDate;
            ...
            let studyTime =  ds.images[0]._study.studyTime;
            ...
            let patientName = ds.images[0]._study.patientName.replace(/\^/g, " ");
            let patientId = ds.images[0]._study.patientId;

            let seriesDescription = ds.images[0]._series.seriesDescription.replace(/\^/g, " ");
            let studyDescription = ds.images[0]._study.studyDescription.replace(/\^/g, " ");

            let vd = publicAPI.getViewDirection();

            let topLeftParent = document.querySelector('#'+ vd +"TopLeft");
            ...
            let botLeftParent = document.querySelector('#'+vd+"BottomLeft");

            var bottomLeftMap = new Map();

            let seriesNum = ds.seriesNumber;
            let columns = ds.images[0]._data.columns;
            let rows = ds.images[0]._data.rows;
            ...
            let topRightParent = document.querySelector('#'+vd+"TopRight");
